...
class MyController(http.Controller): 
    ...
    @http.route('/download/data', type='http', auth='public', 
methods=['GET'], csrf=False) 
    ...
    @http.route('/api/training/course/create', type='http', auth='public', 
methods=['POST'], csrf=False) 
    def training_course_create(self, **kw): 
        try: 
            ...
            return json.dumps({'message': 'Success Create', 'id': 
new_record.id}) 
        except Exception as e: 
            return json.dumps({'message': 'Failed Create', 'error': 
str(e)}) 

    # controller api update data 
    @http.route('/api/training/course/update', type='http', auth='public', 
methods=['POST'], csrf=False) 
...
limit=1) 
    ...
    @http.route('/api/training/course/delete', type='http', auth='public', 
methods=['POST'], csrf=False) 
